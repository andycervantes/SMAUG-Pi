...
void do_goto( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !xIS_SET( ch->act, PLR_WIZINVIS ) )
		act( AT_IMMORT, "$n $T", ch, NULL, ( ch->pcdata && ch->pcdata->bamfout[0] != '\0' ) ? ch->pcdata->bamfout : ( char * ) "leaves in a swirling mist.", TO_ROOM );
	...
	if ( !xIS_SET( ch->act, PLR_WIZINVIS ) )
		act( AT_IMMORT, "$n $T", ch, NULL, ( ch->pcdata && ch->pcdata->bamfin[0] != '\0' ) ? ch->pcdata->bamfin : ( char * ) "appears in a swirling mist.", TO_ROOM );
		...
		if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
		{
			...
			for ( paf = pObjIndex->first_affect; paf; paf = paf->next )
			{
				if ( ++count == loc )
				{
					...
					if ( paf->location != APPLY_WEARSPELL && paf->location != APPLY_REMOVESPELL && paf->location != APPLY_STRIPSN && paf->location != APPLY_WEAPONSPELL )
					{
						CHAR_DATA *vch;
						OBJ_DATA *eq;
...
void add_room_affect( ROOM_INDEX_DATA * location, CHAR_DATA * ch, bool indexaffect, const char *argument )
{
	...
	else if ( loc == APPLY_WEAPONSPELL || loc == APPLY_WEARSPELL || loc == APPLY_REMOVESPELL || loc == APPLY_STRIPSN || loc == APPLY_RECURRINGSPELL )
	{
...
/*	if ( get_trust(ch) >= sysdata.level_modify_proto )   */
	...
	if ( get_trust( ch ) >= LEVEL_SUB_IMPLEM || ( is_name( buf, ch->pcdata->bestowments ) && get_trust( ch ) >= sysdata.level_modify_proto ) )
	...
	if ( !tarea )
		for ( tmp = first_build; tmp; tmp = tmp->next )
			...
			{
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
				if ( get_trust( ch ) >= LEVEL_GREATER || is_name( tmp->filename, ch->pcdata->bestowments ) || ( ch->pcdata->council && is_name( "aassign", ch->pcdata->council->powers ) ) )
				{
					tarea = tmp;
					break;
				}
	...
	if ( paf->type < 0 || paf->type >= num_skills )
	{
		fprintf( fp, "Affect       %d %d %d %d %s\n",
			...
			( ( paf->location == APPLY_WEAPONSPELL
				...
				&& IS_VALID_SN( paf->modifier ) ) ? skill_table[paf->modifier]->slot : paf->modifier, paf->location, print_bitvector( &paf->bitvector ) );
	}
	else
	{
...
void save_reset_level( FILE * fpout, RESET_DATA * start_reset, const int level )
{
	...
	for ( reset = start_reset; reset; )
	{
		switch ( UPPER( reset->command ) )	/* extra arg1 arg2 arg3 */
		{
			...
			default:
				fprintf( fpout, "%*.sReset %c %d %d %d %d\n", spaces, "", UPPER( reset->command ), reset->extra, reset->arg1, reset->arg2, reset->arg3 );
				break;

			case 'G':
			case 'R':
				fprintf( fpout, "%*.sReset %c %d %d %d\n", spaces, "", UPPER( reset->command ), reset->extra, reset->arg1, reset->arg2 );
				break;
...
void fwrite_fuss_object( FILE * fpout, OBJ_INDEX_DATA * pObjIndex, bool install )
{
	...
	fprintf( fpout, "Stats    %d %d %d %d %d\n", pObjIndex->weight,
		pObjIndex->cost, pObjIndex->rent ? pObjIndex->rent : ( int ) ( pObjIndex->cost / 10 ), pObjIndex->level, pObjIndex->layers );
	...
	switch ( pObjIndex->item_type )
	{
		...
		case ITEM_SCROLL:
			fprintf( fpout, "Spells   '%s' '%s' '%s'\n",
				...
				IS_VALID_SN( pObjIndex->value[2] ) ? skill_table[pObjIndex->value[2]]->name : "NONE", IS_VALID_SN( pObjIndex->value[3] ) ? skill_table[pObjIndex->value[3]]->name : "NONE" );
			break;
		case ITEM_STAFF:
		case ITEM_WAND:
		...
		case ITEM_SALVE:
			fprintf( fpout, "Spells   '%s' '%s'\n",
				IS_VALID_SN( pObjIndex->value[4] ) ? skill_table[pObjIndex->value[4]]->name : "NONE", IS_VALID_SN( pObjIndex->value[5] ) ? skill_table[pObjIndex->value[5]]->name : "NONE" );
			break;
	}
...
void fwrite_fuss_mobile( FILE * fpout, MOB_INDEX_DATA * pMobIndex, bool install )
{
	...
	fprintf( fpout, "Stats1     %d %d %d %d %d %d\n", pMobIndex->alignment, pMobIndex->level, pMobIndex->mobthac0, pMobIndex->ac, pMobIndex->gold, pMobIndex->exp );
	...
	if ( ( pShop = pMobIndex->pShop ) != NULL )
	{
		fprintf( fpout, "ShopData   %d %d %d %d %d %d %d %d %d\n",
			pShop->buy_type[0], pShop->buy_type[1], pShop->buy_type[2], pShop->buy_type[3], pShop->buy_type[4], pShop->profit_buy, pShop->profit_sell, pShop->open_hour, pShop->close_hour );
	}
...
void old_fold_area( AREA_DATA * tarea, char *filename, bool install )
{
	...
	for ( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
	{
		...
		if ( pMobIndex->perm_str != 13 || pMobIndex->perm_int != 13
			...
			|| pMobIndex->height != 0 || pMobIndex->weight != 0 || pMobIndex->speaks != 0 || pMobIndex->speaking != 0 || pMobIndex->xflags != 0 || pMobIndex->numattacks != 0 )
			complexmob = TRUE;
		else
			complexmob = FALSE;
		fprintf( fpout, "#%d\n", vnum );
		...
		if ( complexmob )
		{
			...
			fprintf( fpout, "%d %d %d %d %d %d %s ",
				pMobIndex->hitroll, pMobIndex->damroll, pMobIndex->xflags, pMobIndex->resistant, pMobIndex->immune, pMobIndex->susceptible, print_bitvector( &pMobIndex->attacks ) );
	...
	for ( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ )
	{
		...
		fprintf( fpout, "%s~\n", pObjIndex->description );
		fprintf( fpout, "%s~\n", pObjIndex->action_desc );
		fprintf( fpout, "%d %s %d %d %d\n", pObjIndex->item_type, print_bitvector( &pObjIndex->extra_flags ), pObjIndex->wear_flags, pObjIndex->layers, pObjIndex->level );
					...
					for ( pexit = room->first_exit; pexit; pexit = pexit->next )
					{
						if ( pexit->to_room->vnum == exvnum )
							pager_printf( ch, "%5d) Direction: %-2s.  Key: %d Flags: %d Keywords: %s.\n\r",
								room->vnum, dir_text[pexit->vdir], pexit->key, pexit->exit_info, pexit->keyword[0] != '\0' ? pexit->keyword : "(none)" );
					}
				}
			}
	}
				...
				if ( rev ? rexit->to_room->area == target : rexit->to_room->area != target )
				{
					...
					if ( detail == 0 )
						ch_printf( ch, "%s&C%s:&g%d&O%s%s%s", ex_found ? " " : "    ", dirs[dnum], rexit->vnum,
							xIS_EMPTY( rexit->to_room->progtypes ) ? "" : "p", rexit->key <= 0 ? "" : "k", rexit->exit_info ? "f" : "" );
					else if ( detail == 1 )
					{
						ch_printf( ch, "    &C%-2s&c:&g%d&O%s &c- %s%s", dirs[dnum], rexit->vnum,
							xIS_EMPTY( rexit->to_room->progtypes ) ? "" : "p", rev ? "&w" : "&W", rev ? rexit->to_room->name : rexit->to_room->area->name );
						if ( rexit->exit_info )
						...
						ch_printf( ch, "    &C%-2s&c:&g%d&O%s%s%s &c- &W%s &c- &w%s\r\n", dirs[dnum], rexit->vnum,
							xIS_EMPTY( rexit->to_room->progtypes ) ? "" : "p", rexit->key <= 0 ? "" : "k", rexit->exit_info ? "f" : "", rexit->to_room->area->name, rexit->to_room->name );
					ex_found = TRUE;
				}
			}
