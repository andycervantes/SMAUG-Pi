...
{
	if ( xIS_SET( ch->in_room->room_flags, ROOM_NOFLOOR ) && CAN_GO( ch, DIR_DOWN ) && ( !IS_AFFECTED( ch, AFF_FLYING ) || ( ch->mount && !IS_AFFECTED( ch->mount, AFF_FLYING ) ) ) )
	{
		if ( fall > 80 )
		{
...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
	...
	if ( !fall && !IS_NPC( ch ) )
	{
		...
		if ( IS_SET( to_room->area->flags, AFLAG_NOPKILL ) && !IS_SET( ch->in_room->area->flags, AFLAG_NOPKILL ) && ( IS_PKILL( ch ) && !IS_IMMORTAL( ch ) ) )
		{
...
void do_bashdoor( CHAR_DATA * ch, const char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
	{
		...
		if ( !IS_SET( pexit->exit_info, EX_BASHPROOF ) && ch->move >= 15 && number_percent(  ) < ( schance + 4 * ( get_curr_str( ch ) - 19 ) ) )
		{
