			...
			case ITEM_CORPSE_PC:
				{
					...
					if ( IS_OBJ_STAT( container, ITEM_CLANCORPSE )
						&& !IS_NPC( ch ) && !IS_IMMORTAL( ch ) && container->action_desc[0] != '\0' && str_cmp( name, ch->name ) && str_cmp( container->action_desc, ch->name ) )
					{
			...
			if ( !found )
			{
				if ( fAll )
				{
					if ( container->item_type == ITEM_KEYRING && !IS_OBJ_STAT( container, ITEM_COVERING ) )
						...
						act( AT_PLAIN, IS_OBJ_STAT( container, ITEM_COVERING ) ? "I see nothing beneath the $T." : "I see nothing in the $T.", ch, NULL, container->short_descr, TO_CHAR );
				}
				else
				{
					if ( container->item_type == ITEM_KEYRING && !IS_OBJ_STAT( container, ITEM_COVERING ) )
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit )
{
	...
	if ( !IS_IMMORTAL( ch )
		&& ( ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && ch->Class == CLASS_WARRIOR )
			...
			|| ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && ch->Class == CLASS_AUGURER ) || ( IS_OBJ_STAT( obj, ITEM_ANTI_CLERIC ) && ch->Class == CLASS_CLERIC ) ) )
	{
	...
	switch ( 1 << bit )
	{
		...
		case ITEM_WEAR_SHIELD:
			if ( get_eq_char( ch, WEAR_DUAL_WIELD )
				|| ( get_eq_char( ch, WEAR_WIELD ) && get_eq_char( ch, WEAR_MISSILE_WIELD ) ) || ( get_eq_char( ch, WEAR_WIELD ) && get_eq_char( ch, WEAR_HOLD ) ) )
			{
		...
		case ITEM_HOLD:
			if ( get_eq_char( ch, WEAR_DUAL_WIELD ) || ( get_eq_char( ch, WEAR_WIELD ) && ( get_eq_char( ch, WEAR_MISSILE_WIELD ) || get_eq_char( ch, WEAR_SHIELD ) ) ) )
			{
			...
			if ( obj->item_type == ITEM_WAND
				...
				|| obj->item_type == ITEM_BLOOD || obj->item_type == ITEM_PIPE || obj->item_type == ITEM_HERB || obj->item_type == ITEM_KEY || !oprog_use_trigger( ch, obj, NULL, NULL ) )
			{
			...
			switch ( obj->item_type )
			{
				...
				case ITEM_QUIVER:
					ch_printf( ch, "%s appears to %s.\r\n", capitalize( obj->short_descr ),
						...
						obj->value[0] < 500 ? "have a medium to large capacity" : obj->value[0] < 751 ? "have a large capacity" : "have a giant capacity" );
					break;
...
*/
			...
			case ITEM_SCROLL:
				...
				snprintf( buf, MAX_STRING_LENGTH, "A new item is being auctioned: %s at %d gold.", obj->short_descr, auction->starting );
				talk_auction( buf );
