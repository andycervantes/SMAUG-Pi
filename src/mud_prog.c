...
int mprog_do_command( const char *cmnd, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, const void *vo, CHAR_DATA * rndm, bool ignore, bool ignore_ors );
...
bool oprog_wordlist_check( const char *arg, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, const void *vo, int type, OBJ_DATA * iobj );
...
bool rprog_wordlist_check( const char *arg, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, const void *vo, int type, ROOM_INDEX_DATA * room );
  ...
  doneargs:
	...
	*p = '\0';
	...
	if ( !str_cmp( chck, "economy" ) )
	{
		...
		return mprog_veval( ( ( room->area->high_economy > 0 ) ? 1000000000 : 0 ) + room->area->low_economy, opr, atoi( rval ), mob );
	}
	...
	if ( chkchar )
	{
		...
		if ( !str_cmp( chck, "isleader" ) )
		{
			...
			if ( ( temp = get_clan( rval ) ) == NULL )
				return FALSE;
			if ( mprog_seval( chkchar->name, opr, temp->leader, mob ) || mprog_seval( chkchar->name, opr, temp->number1, mob ) || mprog_seval( chkchar->name, opr, temp->number2, mob ) )
				return TRUE;
			else
				return FALSE;
		}
...
void mprog_driver( const char *com_list, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, const void *vo, bool single_step )
{
	...
	while ( TRUE )
	{
		...
		result = mprog_do_command( cmnd, mob, actor, obj, vo, rndm,
			( ifstate[iflevel][IN_IF] && !ifstate[iflevel][DO_IF] ) || ( ifstate[iflevel][IN_ELSE] && !ifstate[iflevel][DO_ELSE] ), ( ignorelevel > 0 ) );
