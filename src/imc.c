...
CHAR_DATA *imc_find_user( const char *name )
{
	...
	{
		if ( ( vch = d->character ? d->character : d->original ) != NULL && !strcasecmp( CH_IMCNAME( vch ), name ) && d->connected == CON_PLAYING )
			return vch;
	}
	return NULL;
}
...
void imc_new_channel( const char *chan, const char *owner, const char *ops, const char *invite, const char *exclude, bool copen, int perm, const char *lname )
{
	IMC_CHANNEL *c;
...
PFUN( imc_recv_emote )
{
	...
	for ( d = first_descriptor; d; d = d->next )
	{
		if ( d->connected == CON_PLAYING && ( ch = d->original ? d->original : d->character ) != NULL && IMCPERM( ch ) >= level )
...
void update_imchistory( IMC_CHANNEL * channel, char *message )
{
	...
	for ( x = 0; x < MAX_IMCHISTORY; x++ )
	{
		if ( channel->history[x] == NULL )
		{
			...
			snprintf( buf, LGST, "~R[%-2.2d/%-2.2d %-2.2d:%-2.2d] ~G%s", local->tm_mon + 1, local->tm_mday, local->tm_hour, local->tm_min, msg );
...
char *imccenterline( const char *string, int length )
{
	 ...
	 */
	snprintf( outbuf, 400, "%*s%s%*s", ( amount / 2 ), "", string, ( ( amount / 2 ) * 2 ) == amount ? ( amount / 2 ) : ( ( amount / 2 ) + 1 ), "" );
...
void imc_process_finger( const char *from, const char *type )
{
	...
	snprintf( buf, IMC_BUFF_SIZE,
		...
		imcperm_names[IMCPERM( victim )], ( IMC_LISTEN( victim ) && IMC_LISTEN( victim )[0] != '\0' ) ? IMC_LISTEN( victim ) : "None" );
	...
	if ( !IMCIS_SET( IMCFLAG( victim ), IMC_PRIVACY ) )
		snprintf( buf + strlen( buf ), IMC_BUFF_SIZE - strlen( buf ),
			...
			( IMC_YAHOO( victim ) && IMC_YAHOO( victim )[0] != '\0' ) ? IMC_YAHOO( victim ) : "None", ( IMC_MSN( victim ) && IMC_MSN( victim )[0] != '\0' ) ? IMC_MSN( victim ) : "None" );
...
void imc_parse_packet( const char *packet )
{
	...
	if ( !pfun )
	{
		if ( imcpacketdebug )
		{
			imclog( "PACKET: From %s, Seq %lu, Route %s, Type %s, To %s, EXTRA %s", p->from, seq, p->route, p->type, p->to, packet );
...
bool imc_server_connect( void )
{
	...
	if ( this_imcmud->sha256 )
	{
		...
		if ( !this_imcmud->sha256pass )
		{
			snprintf( buf, LGST, "PW %s %s version=%d autosetup %s SHA256", this_imcmud->localname, this_imcmud->clientpw, IMC_VERSION, this_imcmud->serverpw );
		}
		/*
	 ...
	 */
		...
		snprintf( buf, LGST, "PW %s %s version=%d autosetup %s", this_imcmud->localname, this_imcmud->clientpw, IMC_VERSION, this_imcmud->serverpw );
...
void imc_hotboot( void )
{
	...
	if ( this_imcmud && this_imcmud->state == IMC_ONLINE )
	{
		...
		{
			fprintf( fp, "%s %s\n", ( this_imcmud->network ? this_imcmud->network : "Unknown" ), ( this_imcmud->servername ? this_imcmud->servername : "Unknown" ) );
			IMCFCLOSE( fp );
			imc_savehistory(  );
		}
	}
...
IMC_CMD( imcchanlist )
{
	...
	for ( c = first_imc_channel; c; c = c->next )
	{
		...
		imc_printf( ch, "~c%-15.15s ~%c%-*.*s ~B%-15.15s ~b%-7s %s\r\n", c->name, col,
			...
			c->local_name ? c->local_name : "~Y(not local)  ", c->owner, imcperm_names[c->level], c->refreshed ? ( c->open ? "~gOpen" : "~yPrivate" ) : "~Runknown" );
...
IMC_CMD( imclist )
{
	...
	for ( p = first_rinfo; p; p = p->next, count++ )
	{
		...
		imcpager_printf( ch, "\r\n~%c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s", p->expired ? 'R' : 'c', p->name, p->version, netname, serverpath );
	}
...
#    if defined(SMAUGSOCIAL)
	...
	{
		if ( person && person[0] != '\0' && !strcasecmp( person, CH_IMCNAME( ch ) ) && mud && mud[0] != '\0' && !strcasecmp( mud, this_imcmud->localname ) )
		{
	...
	while ( *format != '\0' )
	{
		...
		else
		{
			switch ( *format )
			{
				...
				case 'e':
					i = should_upper ? imccapitalize( he_she[URANGE( 0, CH_IMCSEX( ch ), 2 )] ) : he_she[URANGE( 0, CH_IMCSEX( ch ), 2 )];
					break;

				case 'E':
					i = should_upper ? imccapitalize( he_she[URANGE( 0, CH_IMCSEX( vic ), 2 )] ) : he_she[URANGE( 0, CH_IMCSEX( vic ), 2 )];
					break;

				case 'm':
					i = should_upper ? imccapitalize( him_her[URANGE( 0, CH_IMCSEX( ch ), 2 )] ) : him_her[URANGE( 0, CH_IMCSEX( ch ), 2 )];
					break;

				case 'M':
					i = should_upper ? imccapitalize( him_her[URANGE( 0, CH_IMCSEX( vic ), 2 )] ) : him_her[URANGE( 0, CH_IMCSEX( vic ), 2 )];
					break;

				case 's':
					i = should_upper ? imccapitalize( his_her[URANGE( 0, CH_IMCSEX( ch ), 2 )] ) : his_her[URANGE( 0, CH_IMCSEX( ch ), 2 )];
					break;

				case 'S':
					i = should_upper ? imccapitalize( his_her[URANGE( 0, CH_IMCSEX( vic ), 2 )] ) : his_her[URANGE( 0, CH_IMCSEX( vic ), 2 )];
					break;
