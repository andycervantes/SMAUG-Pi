...
void do_watch( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !str_cmp( arg, "site" ) && *arg2 )
	{
		...
		for ( vic = first_char; vic; vic = vic->next )
			if ( !IS_NPC( vic ) && vic->desc && *pinsert->player_site && !str_prefix( pinsert->player_site, vic->desc->host ) && get_trust( vic ) < pinsert->imm_level )
				SET_BIT( vic->pcdata->flags, PCFLAG_WATCH );
...
void do_rstat( CHAR_DATA * ch, const char *argument )
{
	...
	for ( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next )
		ch_printf( ch,
			...
			++cnt, dir_text[pexit->vdir], pexit->to_room ? pexit->to_room->vnum : 0, pexit->key, pexit->exit_info, pexit->keyword[0] != '\0' ? pexit->keyword : "(none)" );
	return;
}
...
void do_ostat( CHAR_DATA * ch, const char *argument )
{
	...
	ch_printf_color( ch, "&cIndex Values : &w%d %d %d %d %d %d.\r\n",
		obj->pIndexData->value[0], obj->pIndexData->value[1], obj->pIndexData->value[2], obj->pIndexData->value[3], obj->pIndexData->value[4], obj->pIndexData->value[5] );
...
void do_mstat( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !IS_NPC( victim ) && victim->pcdata->clan )
		pager_printf_color( ch, "   &c%s: &w%s",
			victim->pcdata->clan->clan_type == CLAN_ORDER ? "Order" : victim->pcdata->clan->clan_type == CLAN_GUILD ? "Guild" : "Clan", victim->pcdata->clan->name );
	send_to_pager( "\r\n", ch );
	if ( get_trust( ch ) >= LEVEL_GOD && !IS_NPC( victim ) && victim->desc )
		pager_printf_color( ch, "&cHost: &w%s   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\r\n",
			victim->desc->host, victim->desc->descriptor, victim->trust, victim->pcdata->authed_by[0] != '\0' ? victim->pcdata->authed_by : "(unknown)" );
	...
	pager_printf_color( ch, "&cclass  : &w%-2.2d/%-10s   &cRace      : &w%-2.2d/%-10s\r\n",
		...
		race_table[victim->race]->race_name && race_table[victim->race]->race_name[0] != '\0' ? race_table[victim->race]->race_name : "unknown" );
	...
	if ( IS_NPC( victim ) )
		pager_printf_color( ch,
			...
			victim->barenumdie, victim->baresizedie, victim->damplus, victim->pIndexData->damnodice, victim->pIndexData->damsizedice, victim->pIndexData->damplus, victim->numattacks );
	pager_printf_color( ch, "&cMentalState: &w%-3d   &cEmotionalState: &w%-3d   ", victim->mental_state, victim->emotional_state );
	if ( !IS_NPC( victim ) )
	...
	pager_printf_color( ch, "&cSave versus: &w%d %d %d %d %d       &cItems: &w(%d/%d)  &cWeight &w(%d/%d)\r\n",
		...
		victim->saving_wand,
		victim->saving_para_petri, victim->saving_breath, victim->saving_spell_staff, victim->carry_number, can_carry_n( victim ), victim->carry_weight, can_carry_w( victim ) );
	...
	else
	{
		...
		if ( victim->pcdata->nuisance )
		{
			pager_printf_color( ch, "&RNuisance   &cStage: (&R%d&c/%d)  Power:  &w%d  &cTime:  &w%s.\r\n",
				victim->pcdata->nuisance->flags, MAX_NUISANCE_STAGE, victim->pcdata->nuisance->power, ctime( &victim->pcdata->nuisance->set_time ) );
		}
	}
	if ( victim->morph )
	{
	...
	for ( obj = first_object; obj; obj = obj->next )
	{
		if ( obj->in_room && obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC && !str_cmp( buf2, obj->short_descr ) )
		{
			...
			pager_printf_color( ch, "&P%s:  %s%-12.12s %s  &PIn:  &w%-22.22s  &P[&w%5d&P]   &PTimer:  %s%2d",
				...
				obj->in_room->area->name, obj->in_room->vnum, obj->timer < 1 ? "&w" : obj->timer < 5 ? "&R" : obj->timer < 10 ? "&Y" : "&w", obj->timer );
			if ( bag )
			{
				obj_from_room( obj );
...
void do_notitle( CHAR_DATA * ch, const char *argument )
{
	...
	else
	{
		...
		snprintf( buf, MAX_STRING_LENGTH, "the %s", title_table[victim->Class][victim->level][victim->sex == SEX_FEMALE ? 1 : 0] );
...
void save_watchlist( void )
{
	...
	for ( pwatch = first_watch; pwatch; pwatch = pwatch->next )
		fprintf( fp, "%d %s~%s~%s~\n", pwatch->imm_level, pwatch->imm_name, pwatch->target_name ? pwatch->target_name : " ", pwatch->player_site ? pwatch->player_site : " " );
	fprintf( fp, "-1\n" );
	fclose( fp );
	return;
}
...
void do_newbieset( CHAR_DATA * ch, const char *argument )
{
	...
	if ( ( victim->Class == CLASS_MAGE ) || ( victim->Class == CLASS_THIEF ) || ( victim->Class == CLASS_VAMPIRE ) || ( victim->Class == CLASS_AUGURER ) )
	{
...
void do_project( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !str_cmp( arg, "more" ) || !str_cmp( arg, "mine" ) )
	{
		...
		{
			pcount++;
			if ( MINE && ( !pproject->owner || str_cmp( ch->name, pproject->owner ) ) && ( !pproject->coder || str_cmp( ch->name, pproject->coder ) ) )
				continue;
			...
			pager_printf( ch, "%2d | %-11s | %-20s | %-13s | %-10s | %3d\r\n",
				pcount,
				pproject->owner ? pproject->owner : "(None)", pproject->name, pproject->coder ? pproject->coder : "(None)", pproject->status ? pproject->status : "(None)", num_logs );
		}
		return;
	}
		...
		for ( pproject = first_project; pproject; pproject = pproject->next )
		{
			...
			if ( !aflag )
				pager_printf( ch, "%2d | %-11s | %-20s | %-24s | %-10s\r\n",
					pcount, pproject->owner ? pproject->owner : "(None)", pproject->name, pproject->date, pproject->status ? pproject->status : "(None)" );
	...
	if ( !str_cmp( arg1, "status" ) )
	{
		if ( pproject->owner && str_cmp( pproject->owner, ch->name ) &&
			get_trust( ch ) < LEVEL_GREATER && pproject->coder && str_cmp( pproject->coder, ch->name ) && str_cmp( ch->pcdata->council_name, "Code Council" ) )
		{
	...
	if ( !str_cmp( arg1, "log" ) )
	{
		...
		if ( !str_cmp( arg2, "post" ) )
		{
			...
			if ( pproject->owner && str_cmp( ch->name, pproject->owner ) &&
				pproject->coder && str_cmp( ch->name, pproject->coder ) && get_trust( ch ) < LEVEL_GREATER && str_cmp( ch->pcdata->council_name, "Code Council" ) )
			{
				ch_printf( ch, "This is not your project!\r\n" );
				return;
			}
		...
		if ( !str_cmp( arg2, "list" ) )
		{
			if ( pproject->owner && pproject->coder &&
				str_cmp( ch->name, pproject->owner ) && get_trust( ch ) < LEVEL_SAVIOR && str_cmp( ch->name, pproject->coder ) && str_cmp( ch->pcdata->council_name, "Code Council" ) )
			{
				ch_printf( ch, "This is not your project!\r\n" );
				return;
			}
		...
		{
			if ( pproject->owner && str_cmp( ch->name, pproject->owner ) &&
				get_trust( ch ) < LEVEL_ASCENDANT && pproject->coder && str_cmp( ch->name, pproject->coder ) && str_cmp( ch->pcdata->council_name, "Code Council" ) )
			{
				ch_printf( ch, "This is not your project!\r\n" );
				return;
			}
...
void do_ipcompare( CHAR_DATA * ch, const char *argument )
{
	...
	else if ( !str_cmp( arg, "pkill" ) )
	{
		...
		for ( d = first_descriptor; d; d = d->next )
		{
			...
			if ( ( d->connected != CON_PLAYING && d->connected != CON_EDITING ) || d->character == NULL || !CAN_PKILL( d->character ) || !can_see( ch, d->character ) )
				continue;
			CREATE( temp, IPCOMPARE_DATA, 1 );
			temp->host = str_dup( d->host );
			...
			temp->idle = d->idle;
			temp->port = d->port;
			temp->name = ( d->original ? str_dup( d->original->name ) : d->character ? str_dup( d->character->name ) : str_dup( "(none)" ) );
			temp->count = 0;
			temp->printed = FALSE;
			LINK( temp, first_ip, last_ip, next, prev );
		}
		...
		{
			fMatch = FALSE;
			if ( ( d->connected != CON_PLAYING && d->connected != CON_EDITING ) || d->character == NULL || !can_see( ch, d->character ) )
				continue;
			for ( hmm = first_ip; hmm; hmm = hmm->next )
			{
				if ( !str_cmp( hmm->host, d->host ) && str_cmp( hmm->name, ( d->original ? d->original->name : d->character ? d->character->name : "(none)" ) ) )
				{
					fMatch = TRUE;
					break;
				}
			...
			if ( fMatch && hmm )
			{
				...
				{
					snprintf( buf, MAX_STRING_LENGTH, " %3d| %2d|%4d|%6d| %-12s", hmm->descriptor, hmm->connected, hmm->idle / 4, hmm->port, hmm->name );
					if ( get_trust( ch ) >= LEVEL_SAVIOR )
				...
				snprintf( buf, MAX_STRING_LENGTH,
					" %3d| %2d|%4d|%6d| %-12s", d->descriptor, d->connected, d->idle / 4, d->port, d->original ? d->original->name : d->character ? d->character->name : "(none)" );
				if ( get_trust( ch ) >= LEVEL_SAVIOR )
...
void do_pcrename( CHAR_DATA * ch, const char *argument )
{
	...
	snprintf( oldname, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( victim->pcdata->filename[0] ), capitalize( victim->pcdata->filename ) );
	snprintf( backname, MAX_STRING_LENGTH, "%s%c/%s", BACKUP_DIR, tolower( victim->pcdata->filename[0] ), capitalize( victim->pcdata->filename ) );
