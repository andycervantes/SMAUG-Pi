...
void do_make( CHAR_DATA * ch, const char *argument )
{
	...
	if ( str_cmp( ch->name, clan->leader ) && str_cmp( ch->name, clan->deity ) && ( clan->clan_type != CLAN_GUILD || str_cmp( ch->name, clan->number1 ) ) )
	{
		send_to_char( "Huh?\r\n", ch );
		return;
	}
...
void do_showclan( CHAR_DATA * ch, const char *argument )
{
	...
	ch_printf_color( ch, "\r\n&w%s    : &W%s\t\t&wBadge: %s\r\n&wFilename : &W%s\r\n&wMotto    : &W%s\r\n",
		clan->clan_type == CLAN_ORDER ? "Order" : ( clan->clan_type == CLAN_GUILD ? "Guild" : "Clan " ), clan->name, clan->badge ? clan->badge : "(not set)", clan->filename, clan->motto );
	...
	if ( clan->clan_type == CLAN_GUILD )
		ch_printf_color( ch, "   &wClass  : &W%d &w(&W%s&w)", clan->Class, clan->Class < MAX_PC_CLASS ? class_table[clan->Class]->who_name : "unknown" );
	send_to_char( "\r\n", ch );
...
void do_orders( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !str_cmp( ch->name, order->deity ) || !str_cmp( ch->name, order->leader ) || !str_cmp( ch->name, order->number1 ) || !str_cmp( ch->name, order->number2 ) )
		ch_printf( ch, "Members    :  %d\r\n", order->members );
	set_char_color( AT_DGREEN, ch );
	ch_printf( ch, "\r\nDescription:\r\n%s\r\n", order->description );
	return;
...
void do_guilds( CHAR_DATA * ch, const char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
		...
		for ( guild = first_clan; guild; guild = guild->next )
			...
			{
				++count;
				ch_printf( ch, "%-20s   %-14s     %-6d       %6d\r\n", guild->name, guild->leader, guild->mkills, guild->mdeaths );
			}
		set_char_color( AT_HUNGRY, ch );
		if ( !count )
	...
	if ( !str_cmp( ch->name, guild->deity ) || !str_cmp( ch->name, guild->leader ) || !str_cmp( ch->name, guild->number1 ) || !str_cmp( ch->name, guild->number2 ) )
		ch_printf( ch, "Members:   %d\r\n", guild->members );
	set_char_color( AT_HUNGRY, ch );
