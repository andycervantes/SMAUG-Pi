...
/*      bool listall; */
		...
		{
			if ( obj->wear_loc == WEAR_NONE && can_see_obj( ch, obj ) && ( cost = get_cost( ch, keeper, obj, TRUE ) ) > 0 && ( arg[0] == '\0' || nifty_is_name( arg, obj->name ) ) )
			{
				if ( !found )
				{
					found = TRUE;
...
void repair_one_obj( CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, const char *arg, int maxgold, const char *fixstr, const char *fixstr2 )
{
	...
	{
		snprintf( buf, MAX_STRING_LENGTH, "$N tells you, 'It will cost %d piece%s of gold to %s %s...'", cost, cost == 1 ? "" : "s", fixstr, obj->name );
...
void appraise_all( CHAR_DATA * ch, CHAR_DATA * keeper, const char *fixstr )
{
	...
	{
		send_to_char( "\r\n", ch );
		snprintf( buf, MAX_STRING_LENGTH, "$N tells you, 'It will cost %d piece%s of gold in total.'", total, cost == 1 ? "" : "s" );
		act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
...
void do_appraise( CHAR_DATA * ch, const char *argument )
{
	...
	snprintf( buf, MAX_STRING_LENGTH, "$N tells you, 'It will cost %d piece%s of gold to %s that...'", cost, cost == 1 ? "" : "s", fixstr );
	act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
	if ( cost > ch->gold )
		act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch, NULL, keeper, TO_CHAR );
