...
void do_hotboot( CHAR_DATA * ch, const char *argument )
{
	...
	for ( d = first_descriptor; d; d = d->next )
	{
		if ( ( d->connected == CON_PLAYING || d->connected == CON_EDITING )
			&& ( victim = d->character ) != NULL && !IS_NPC( victim ) && victim->in_room && victim->fighting && victim->level >= 1 && victim->level <= MAX_LEVEL )
		{
			found = TRUE;
			count++;
		}
	...
	for ( d = first_descriptor; d; d = de_next )
	{
		...
		{
			fprintf( fp, "%d %d %d %d %d %s %s\n", d->descriptor, d->can_compress, och->in_room->vnum, d->port, d->idle, och->name, d->host );
			/*
