...
static char *print_big_board( CHAR_DATA * ch, GAME_BOARD_DATA * board )
{
	...
	for ( x = 0; x < 8; x++ )
	{
		...
		{
			sprintf( buf, "%s%s",
				x % 2 == 0 ? ( y % 2 == 0 ? BLACK_BACKGROUND : WHITE_BACKGROUND ) : ( y % 2 == 0 ? WHITE_BACKGROUND : BLACK_BACKGROUND ), big_pieces[board->board[x][y]][0] );
		...
		{
			sprintf( buf, "%s%s",
				x % 2 == 0 ? ( y % 2 == 0 ? BLACK_BACKGROUND : WHITE_BACKGROUND ) : ( y % 2 == 0 ? WHITE_BACKGROUND : BLACK_BACKGROUND ), big_pieces[board->board[x][y]][1] );
			sprintf( buf2, buf, IS_WHITE( board->board[x][y] ) ? s1 : s2 );
			strcat( retbuf, buf2 );
		}
		strcat( retbuf, BLACK_BACKGROUND "\n\r" );
...
PFUN( imc_recv_chess )
{
	...
	{
		if ( !victim->pcdata->game_board ||
			victim->pcdata->game_board->player2 == NULL || victim->pcdata->game_board->type != TYPE_IMC || str_cmp( victim->pcdata->game_board->player2, q->from ) )
		{
			imc_send_chess( victim->pcdata->game_board->player1 ? victim->pcdata->game_board->player1 : NULL, q->from, "stop" );
			return;
		}
	...
	if ( !str_prefix( "move", txt ) )
	{
		...
		{
			imc_send_chess( victim->pcdata->game_board->player1 ? victim->pcdata->game_board->player1 : NULL, q->from, "invalidmove" );
			return;
		}
