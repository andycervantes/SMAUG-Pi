...
void load_smaugwiz_reset( ROOM_INDEX_DATA * room, FILE * fp )
{
	...
	switch ( letter2 )
	{
		...
		case 'D':
			if ( arg3 < 0 || arg3 > MAX_DIR + 1 || ( pexit = get_exit( room, arg3 ) ) == NULL || !IS_SET( pexit->exit_info, EX_ISDOOR ) )
			{
		...
		switch ( UPPER( word[0] ) )
		{
			...
			case 'E':
				if ( !str_cmp( word, "End" ) )
				{
					...
					{
						snprintf( checkname, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( lmsg->name[0] ), capitalize( lmsg->name ) );
