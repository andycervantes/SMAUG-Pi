...
void do_cast( CHAR_DATA * ch, const char *argument )
{
	...
	switch ( ch->substate )
	{
		default:
			...
			if ( !IS_NPC( ch ) && victim && !IS_NPC( victim ) && CAN_PKILL( victim ) && !CAN_PKILL( ch ) && !in_arena( ch ) && !in_arena( victim ) )
			{
				set_char_color( AT_MAGIC, ch );
...
ch_ret spell_charm_person( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( IS_AFFECTED( victim, AFF_CHARM )
		|| schance == 1000 || IS_AFFECTED( ch, AFF_CHARM ) || level < victim->level || circle_follow( victim, ch ) || !can_charm( ch ) || saves_spell_staff( schance, victim ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
	...
	for ( obj = first_object; obj; obj = obj->next )
	{
		...
		if ( in_obj->carried_by )
		{
			if ( IS_IMMORTAL( in_obj->carried_by )
				&& !IS_NPC( in_obj->carried_by ) && ( get_trust( ch ) < in_obj->carried_by->pcdata->wizinvis ) && xIS_SET( in_obj->carried_by->act, PLR_WIZINVIS ) )
			{
				found--;
				continue;
			}
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( IS_AFFECTED( victim, AFF_SLEEP )
		...
		|| level < victim->level || ( victim != ch && xIS_SET( victim->in_room->room_flags, ROOM_SAFE ) ) || saves_spell_staff( schance, victim ) )
	{
		failed_casting( skill, ch, victim, NULL );
		if ( ch == victim )
			return rSPELL_FAILED;
...
ch_ret spell_summon( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( ( victim = get_char_world( ch, target_name ) ) == NULL
		...
		|| ( IS_SET( ch->in_room->area->flags, AFLAG_NOPKILL ) && IS_PKILL( victim ) ) || ( !IS_NPC( ch ) && !IS_NPC( victim ) && IS_SET( victim->pcdata->flags, PCFLAG_NOSUMMON ) ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
...
ch_ret spell_astral_walk( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( ( victim = get_char_world( ch, target_name ) ) == NULL || !can_astral( ch, victim ) || !in_hard_range( ch, victim->in_room->area ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( !victim->in_room
		|| xIS_SET( victim->in_room->room_flags, ROOM_NO_RECALL )
		|| ( !IS_NPC( ch ) && victim->fighting ) || ( victim != ch && ( saves_spell_staff( level, victim ) || saves_wands( level, victim ) ) ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
	...
	for ( ;; )
	{
		...
		if ( pRoomIndex )
			if ( !xIS_SET( pRoomIndex->room_flags, ROOM_PRIVATE )
				...
				&& !xIS_SET( pRoomIndex->room_flags, ROOM_PROTOTYPE ) && !xIS_SET( pRoomIndex->room_flags, ROOM_NO_RECALL ) && in_hard_range( ch, pRoomIndex->area ) )
				break;
	}
...
ch_ret spell_portal( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( ( victim = get_char_world( ch, target_name ) ) == NULL
		...
		|| ( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
		|| ( IS_NPC( victim ) && saves_spell_staff( level, victim ) ) || ( !IS_NPC( victim ) && CAN_PKILL( ch ) != CAN_PKILL( victim ) ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
...
ch_ret spell_farsight( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( ( victim = get_char_world( ch, target_name ) ) == NULL
		...
		|| victim->level >= level + 15
		|| ( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
		|| ( IS_NPC( victim ) && saves_spell_staff( level, victim ) ) || ( !IS_NPC( victim ) && CAN_PKILL( victim ) && !CAN_PKILL( ch ) ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
...
ch_ret spell_plant_pass( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( ( victim = get_char_world( ch, target_name ) ) == NULL
		...
		|| ( IS_NPC( victim ) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
		|| ( IS_NPC( victim ) && saves_spell_staff( level, victim ) )
		|| !in_hard_range( ch, victim->in_room->area ) || ( IS_SET( victim->in_room->area->flags, AFLAG_NOPKILL ) && IS_PKILL( ch ) ) )
	{
		failed_casting( skill, ch, victim, NULL );
		return rSPELL_FAILED;
	}
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( IS_IMMORTAL( ch ) || ( chance( ch, 75 ) && pMobIndex->level - ch->level < 10 ) )
	{
		...
		if ( !pMobIndex->hitnodice )
			mob->max_hit = pMobIndex->level * 8 + number_range( pMobIndex->level * pMobIndex->level / 4, pMobIndex->level * pMobIndex->level );
		else
...
ch_ret spell_attack( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( retcode == rNONE && skill->first_affect
		&& !char_died( ch ) && !char_died( victim ) && ( !is_affected( victim, sn ) || SPELL_FLAG( skill, SF_ACCUMULATIVE ) || SPELL_FLAG( skill, SF_RECASTABLE ) ) )
		retcode = spell_affectchar( sn, level, ch, victim );
	return retcode;
}
...
ch_ret spell_affect( int sn, int level, CHAR_DATA * ch, void *vo )
{
	...
	if ( !groupsp && !areasp )
	{
		...
		if ( ( saf = skill->first_affect ) && !saf->next && saf->location == APPLY_STRIPSN && !is_affected( victim, dice_parse( ch, level, saf->modifier ) ) )
		{
			failed_casting( skill, ch, victim, NULL );
			return rSPELL_FAILED;
		}
	...
	for ( ; victim; victim = victim->next_in_room )
	{
		if ( groupsp || areasp )
		{
			if ( ( groupsp && !is_same_group( victim, ch ) )
				...
				|| is_immune( victim, SPELL_DAMAGE( skill ) ) || check_save( sn, level, ch, victim ) || ( !SPELL_FLAG( skill, SF_RECASTABLE ) && is_affected( victim, sn ) ) )
				continue;
	...
	switch ( skill->target )
	{
		case TAR_IGNORE:
			...
			if ( SPELL_FLAG( skill, SF_DISTANT )
				&& ( victim = get_char_world( ch, target_name ) ) && !xIS_SET( victim->in_room->room_flags, ROOM_NO_ASTRAL ) && SPELL_FLAG( skill, SF_CHARACTER ) )
