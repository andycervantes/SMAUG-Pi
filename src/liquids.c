	...
	switch ( obj->item_type )
	{
		...
		case ITEM_BLOOD:
			if ( IS_VAMPIRE( ch ) && !IS_NPC( ch ) )
			{
				...
				{
					if ( ch->pcdata->condition[COND_FULL] >= MAX_COND_VALUE || ch->pcdata->condition[COND_THIRST] >= MAX_COND_VALUE )
					{
		...
		for ( source = ch->in_room->first_content; source; source = src_next )
		{
			...
			if ( dest_item == ITEM_CONTAINER )
			{
				if ( !CAN_WEAR( source, ITEM_TAKE ) || IS_OBJ_STAT( source, ITEM_BURIED )
					...
					|| ch->carry_weight + get_obj_weight( source ) > can_carry_w( ch ) || ( get_real_obj_weight( source ) + get_real_obj_weight( obj ) / obj->count ) > obj->value[0] )
					continue;
			...
			case ITEM_CORPSE_NPC:
				...
				for ( ; otmp; otmp = otmp_next )
				{
					...
					if ( !CAN_WEAR( otmp, ITEM_TAKE ) || ( IS_OBJ_STAT( otmp, ITEM_PROTOTYPE ) && !can_take_proto( ch ) )
						...
						|| ch->carry_weight + get_obj_weight( otmp ) > can_carry_w( ch ) || ( get_real_obj_weight( source ) + get_real_obj_weight( obj ) / obj->count ) > obj->value[0] )
						continue;
