...
void do_showdeity( CHAR_DATA * ch, const char *argument )
{
	...
	ch_printf( ch, "Race: %-12sClass: %-11sSex: %-13sRace2: %s\r\n",
		...
		deity->sex == -1 ? "none" :
		deity->sex == SEX_MALE ? "male" : deity->sex == SEX_FEMALE ? "female" : "neutral", ( deity->race2 < 0 || deity->race2 > MAX_PC_RACE ) ? "none" : npc_race[deity->race2] );
	...
	if ( !str_cmp( arg, "corpse" ) && !IS_PKILL( ch ) )
	{
		...
		if ( ( oldfavor > ch->pcdata->deity->affectednum &&
			...
			( oldfavor > ch->pcdata->deity->elementnum &&
				ch->pcdata->favor <= ch->pcdata->deity->elementnum ) || ( oldfavor < ch->pcdata->deity->susceptnum && ch->pcdata->favor >= ch->pcdata->deity->susceptnum ) )
		{
			update_aris( ch );
		}
...
void adjust_favor( CHAR_DATA * ch, int field, int mod )
{
	...
	if ( ( ch->alignment - ch->pcdata->deity->alignment > 650 || ch->alignment - ch->pcdata->deity->alignment < -650 ) && ch->pcdata->deity->alignment != 0 )
	{
		ch->pcdata->favor -= 2;
