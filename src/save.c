...
void save_char_obj( CHAR_DATA * ch )
{
	...
	snprintf( strsave, MAX_INPUT_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( ch->pcdata->filename[0] ), capitalize( ch->pcdata->filename ) );
	...
	{
		snprintf( strback, MAX_INPUT_LENGTH, "%s%c/%s", BACKUP_DIR, tolower( ch->pcdata->filename[0] ), capitalize( ch->pcdata->filename ) );
...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
	...
	if ( ch->pcdata && ch->pcdata->nuisance )
		fprintf( fp, "NuisanceNew %ld %ld %d %d\n", ch->pcdata->nuisance->set_time, ch->pcdata->nuisance->max_time, ch->pcdata->nuisance->flags, ch->pcdata->nuisance->power );
...
void fwrite_obj( CHAR_DATA * ch, OBJ_DATA * obj, FILE * fp, int iNest, short os_type, bool hotboot )
{
	...
	if ( !hotboot )
	{
		if ( ( ch && ch->level < obj->level ) || ( obj->item_type == ITEM_KEY && !IS_OBJ_STAT( obj, ITEM_CLANOBJECT ) ) || obj_extracted( obj ) || IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
			return;
	}
...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover )
{
	...
	if ( stat( strsave, &fst ) != -1 )
	{
		...
		{
			log_printf_plus( LOG_COMM, LEVEL_GREATER, "%s player data for: %s (%dK)", preload ? "Preloading" : "Loading", ch->pcdata->filename, ( int ) fst.st_size / 1024 );
		}
	}
	/*
	 * else no player file 
