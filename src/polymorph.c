...
void do_morphstat( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !argument || argument[0] == '\0' )
	{
		...
		pager_printf( ch, "  &cSex:  &C%s   &cPkill:   &C%s   &cTime From:   &C%d   &cTime To:    &C%d\r\n",
			...
			morph->sex == SEX_FEMALE ? "female" : "neutral", morph->pkill == ONLY_PKILL ? "YES" : morph->pkill == ONLY_PEACEFULL ? "NO" : "n/a", morph->timefrom, morph->timeto );
	...
	else if ( !str_cmp( argument, "help" ) || !str_cmp( argument, "desc" ) )
	{
		...
		pager_printf( ch, "  &cLongdesc    : &w%s", ( morph->long_desc && morph->long_desc[0] == '\0' ) ? "(none set)\r\n" : morph->long_desc );
