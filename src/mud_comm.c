...
void do_mptransfer( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !str_cmp( arg1, "area" ) )
	{
		...
		{
			if ( !d->character || ( d->connected != CON_PLAYING && d->connected != CON_EDITING ) || ch->in_room->area != d->character->in_room->area )
				continue;
...
void do_mp_log( CHAR_DATA * ch, const char *argument )
{
	...
	snprintf( buf, MAX_STRING_LENGTH, "&p%-2.2d/%-2.2d | %-2.2d:%-2.2d  &P%s:  &p%s", t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, ch->short_descr, argument );
