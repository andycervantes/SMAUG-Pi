...
void print_compass( CHAR_DATA * ch )
{
	...
	{
		if ( !pexit->to_room || IS_SET( pexit->exit_info, EX_HIDDEN ) || ( IS_SET( pexit->exit_info, EX_SECRET ) && IS_SET( pexit->exit_info, EX_CLOSED ) ) )
			continue;
	...
	ch_printf_color( ch, "\r\n%-50s         %s%s    %s%s    %s%s\r\n",
		...
		exit_colors[exit_info[DIR_NORTH]], exit_info[DIR_NORTH] ? "N" : "-", exit_colors[exit_info[DIR_NORTHEAST]], exit_info[DIR_NORTHEAST] ? "NE" : " -" );
	...
	ch_printf_color( ch, "%s%s&w<-%s%s&w-&W(*)&w-%s%s&w->%s%s\r\n", exit_colors[exit_info[DIR_WEST]],
		...
		exit_colors[exit_info[DIR_DOWN]], exit_info[DIR_DOWN] ? "D" : "-", exit_colors[exit_info[DIR_EAST]], exit_info[DIR_EAST] ? "E" : "-" );
...
void do_look( CHAR_DATA * ch, const char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg1, TRUE ) ) != NULL )
	{
		if ( IS_SET( pexit->exit_info, EX_CLOSED ) && !IS_SET( pexit->exit_info, EX_WINDOW ) )
		{
			...
			{
				if ( pexit->keyword[strlen( pexit->keyword ) - 1] == 's' || ( pexit->keyword[strlen( pexit->keyword ) - 1] == '\'' && pexit->keyword[strlen( pexit->keyword ) - 2] == 's' ) )
		 ...
		 */
		if ( pexit->to_room && ( IS_AFFECTED( ch, AFF_SCRYING ) || ch->Class == CLASS_THIEF || IS_SET( pexit->exit_info, EX_xLOOK ) || get_trust( ch ) >= LEVEL_IMMORTAL ) )
		{
...
void do_exits( CHAR_DATA * ch, const char *argument )
{
	...
	if ( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_HOLYLIGHT ) && !IS_AFFECTED( ch, AFF_TRUESIGHT ) && !IS_AFFECTED( ch, AFF_INFRARED ) && room_is_dark( ch->in_room ) )
	{
		set_char_color( AT_DGREY, ch );
		send_to_char( "It is pitch black ... \r\n", ch );
		return;
	...
	{
		if ( pexit->to_room
			&& !IS_SET( pexit->exit_info, EX_CLOSED ) && ( !IS_SET( pexit->exit_info, EX_WINDOW ) || IS_SET( pexit->exit_info, EX_ISDOOR ) ) && !IS_SET( pexit->exit_info, EX_HIDDEN ) )
		{
			found = TRUE;
			if ( fAuto )
			{
...
/* start from last to first to get it in the proper order */
	...
	for ( ; d; whogr_p = whogr, whogr = ( fGroup ? whogr->next : NULL ), d = ( fGroup ? ( whogr ? whogr->d : NULL ) : d->prev ) )
	{
		...
		if ( fLeader && !( wch->pcdata->council &&
			...
			!( wch->pcdata->clan && ( ( wch->pcdata->clan->deity &&
					...
					|| ( wch->pcdata->clan->number1
						&& !str_cmp( wch->pcdata->clan->number1, wch->name ) ) || ( wch->pcdata->clan->number2 && !str_cmp( wch->pcdata->clan->number2, wch->name ) ) ) ) )
			continue;
		...
		snprintf( class_text, MAX_INPUT_LENGTH, "%s%2d %s", NOT_AUTHED( wch ) ? "N" : " ", wch->level, class_table[wch->Class]->who_name );
		Class = class_text;
		switch ( wch->level )
		{
			default:
		...
		if ( IS_RETIRED( wch ) )
			...
			Class = "Guest";
		else if ( wch->pcdata->clan && !str_cmp( wch->name, wch->pcdata->clan->leader ) && wch->pcdata->clan->leadrank[0] != '\0' )
			Class = wch->pcdata->clan->leadrank;
		else if ( wch->pcdata->clan && !str_cmp( wch->name, wch->pcdata->clan->number1 ) && wch->pcdata->clan->onerank[0] != '\0' )
			Class = wch->pcdata->clan->onerank;
		else if ( wch->pcdata->clan && !str_cmp( wch->name, wch->pcdata->clan->number2 ) && wch->pcdata->clan->tworank[0] != '\0' )
		...
		snprintf( buf, MAX_STRING_LENGTH, "%*s%-15s %s%s%s%s%s%s%s%s.%s%s%s\r\n",
			...
			xIS_SET( wch->act, PLR_KILLER ) ? "(KILLER) " : "", xIS_SET( wch->act, PLR_THIEF ) ? "(THIEF) " : "", char_name, wch->pcdata->title, extra_title, clan_name, council_name );
	...
	else
	{
		...
		for ( victim = first_char; victim; victim = victim->next )
			if ( victim->in_room
				...
				&& !IS_AFFECTED( victim, AFF_HIDE ) && !IS_AFFECTED( victim, AFF_SNEAK ) && can_see( ch, victim ) && is_name( arg, victim->name ) )
			{
				found = TRUE;
...
void do_practice( CHAR_DATA * ch, const char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
		...
		for ( sn = 0; sn < num_skills; ++sn )
		{
			...
			if ( !IS_IMMORTAL( ch ) && ( skill->guild != CLASS_NONE && ( !IS_GUILDED( ch ) || ( ch->pcdata->clan->Class != skill->guild ) ) ) )
				continue;
	...
	else
	{
		...
		if ( can_prac && ( ( sn == -1 ) || ( !IS_NPC( ch ) && ch->level < skill_table[sn]->skill_level[ch->Class] && ch->level < skill_table[sn]->race_level[ch->race] ) ) )
		{
...
/*          send_to_char_color( !IS_SET( ch->deaf, CHANNEL_PRAY )       ?
	...
	else
	{
		...
		if ( !str_cmp( arg + 1, "auction" ) && get_trust( ch ) > 2 && !NOT_AUTHED( ch ) )
			...
			bit = CHANNEL_CHAT;
		else if ( !str_cmp( arg + 1, "clan" ) && !IS_NPC( ch ) && ch->pcdata->clan && ch->pcdata->clan->clan_type != CLAN_ORDER && ch->pcdata->clan->clan_type != CLAN_GUILD )
			bit = CHANNEL_CLAN;
		else if ( !str_cmp( arg + 1, "council" ) && !IS_NPC( ch ) && ch->pcdata->council )
			bit = CHANNEL_COUNCIL;
		else if ( !str_cmp( arg + 1, "guild" ) && !IS_NPC( ch ) && ch->pcdata->clan && ch->pcdata->clan->clan_type == CLAN_GUILD )
			bit = CHANNEL_GUILD;
			...
			bit = CHANNEL_BUILD;
		else if ( !str_cmp( arg + 1, "high" ) && get_trust( ch ) >= sysdata.log_level && get_trust( ch ) >= sysdata.think_level )
			bit = CHANNEL_HIGH;
		/*
...
*/
			...
			bit = CHANNEL_AUTH;
		else if ( !str_cmp( arg + 1, "newbie" ) && ( IS_IMMORTAL( ch ) || ( ch->pcdata->council && !str_cmp( ch->pcdata->council->name, "Newbie Council" ) ) ) )
			bit = CHANNEL_NEWBIE;
...
void do_config( CHAR_DATA * ch, const char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		...
		ch_printf( ch, "%-12s   %-12s   %-12s   %-12s\r\n           %-12s   %-12s   %-12s   %-12s\r\n           %-12s   %-12s",
			...
			: "[-] rip", xIS_SET( ch->act, PLR_COMPASS ) ? "[+] COMPASS" : "[-] compass", xIS_SET( ch->act, PLR_AUTOMAP ) ? "[+] AUTOMAP" : "[-] automap" );
		set_char_color( AT_DGREEN, ch );
		send_to_char( "\r\n\r\nAuto:      ", ch );
		set_char_color( AT_GREY, ch );
		ch_printf( ch, "%-12s   %-12s   %-12s   %-12s",
			...
			: "[-] autogold", xIS_SET( ch->act, PLR_AUTOLOOT ) ? "[+] AUTOLOOT" : "[-] autoloot", xIS_SET( ch->act, PLR_AUTOEXIT ) ? "[+] AUTOEXIT" : "[-] autoexit" );

		set_char_color( AT_DGREEN, ch );
		send_to_char( "\r\n\r\nSafeties:  ", ch );
		set_char_color( AT_GREY, ch );
		ch_printf( ch, "%-12s   %-12s",
			IS_SET( ch->pcdata->flags, PCFLAG_NORECALL ) ? "[+] NORECALL" : "[-] norecall", IS_SET( ch->pcdata->flags, PCFLAG_NOSUMMON ) ? "[+] NOSUMMON" : "[-] nosummon" );

		if ( !IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) )
			ch_printf( ch, "   %-12s   %-12s", xIS_SET( ch->act, PLR_SHOVEDRAG ) ? "[+] DRAG" : "[-] drag", xIS_SET( ch->act, PLR_NICE ) ? "[+] NICE" : "[-] nice" );
		...
		ch_printf( ch, "%-12s   %-12s   %-12s",
			...
			: "[-] telnetga", IS_SET( ch->pcdata->flags, PCFLAG_GROUPWHO ) ? "[+] GROUPWHO" : "[-] groupwho", IS_SET( ch->pcdata->flags, PCFLAG_NOINTRO ) ? "[+] NOINTRO" : "[-] nointro" );
...
void do_whois( CHAR_DATA * ch, const char *argument )
{
	...
	pager_printf( ch, "\r\n'%s%s.'\r\n %s is a %s level %d %s %s, %d years of age.\r\n",
		...
		victim->sex == SEX_FEMALE ? "female" : "neutral", victim->level, capitalize( race_table[victim->race]->race_name ), class_table[victim->Class]->who_name, calculate_age( victim ) );

	pager_printf( ch, " %s is a %sdeadly player", victim->sex == SEX_MALE ? "He" : victim->sex == SEX_FEMALE ? "She" : "It", IS_SET( victim->pcdata->flags, PCFLAG_DEADLY ) ? "" : "non-" );
