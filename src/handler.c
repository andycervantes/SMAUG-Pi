...
void trworld_char_check( CHAR_DATA * rmv )
{
	...
	for ( sp = trw_heap + trw_loops - 1; sp >= trw_heap; --sp )
	{
		...
		if ( sp->next == rmv )
			sp->next = sp->type == TR_CHAR_WORLD_FORW ? ( ( CHAR_DATA * ) ( sp->next ) )->next : ( ( CHAR_DATA * ) ( sp->next ) )->prev;
	}
}
...
void trworld_obj_check( OBJ_DATA * rmv )
{
	...
	for ( sp = trw_heap + trw_loops - 1; sp >= trw_heap; --sp )
	{
		...
		if ( sp->next == rmv )
			sp->next = sp->type == TR_OBJ_WORLD_FORW ? ( ( CHAR_DATA * ) ( sp->next ) )->next : ( ( CHAR_DATA * ) ( sp->next ) )->prev;
	}
}
...
bool can_astral( CHAR_DATA * ch, CHAR_DATA * victim )
{
	if ( victim == ch
		...
		|| ( IS_NPC( victim ) && saves_spell_staff( ch->level, victim ) ) || ( IS_SET( victim->in_room->area->flags, AFLAG_NOPKILL ) && IS_PKILL( ch ) ) )
		return FALSE;
	else
		return TRUE;
}
...
void check_switch( CHAR_DATA * ch, bool possess )
{
	...
	for ( hash = 0; hash < 126; hash++ )
	{
		for ( cmd = command_hash[hash]; cmd; cmd = cmd->next )
		{
			...
			if ( !IS_NPC( ch ) && ch->pcdata->bestowments && is_name( cmd->name, ch->pcdata->bestowments ) && cmd->level <= trust + sysdata.bestow_dif )
				return;
		}
	}
