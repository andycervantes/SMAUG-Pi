	...
	else
	{
		...
		if ( skill->type != SKILL_HERB )
		{
			...
			else
			{
				...
				{
					snprintf( buf, MAX_STRING_LENGTH, "%8.8s) lvl: %3d max: %2d%%", race_table[iRace]->race_name, skill->race_level[iRace], skill->race_adept[iRace] );
...
void do_sset( CHAR_DATA * ch, const char *argument )
{
	...
	if ( arg1[0] == 'h' )
		...
		sn = atoi( arg1 );
	if ( get_trust( ch ) > LEVEL_GREATER && ( ( arg1[0] == 'h' && is_number( arg1 + 1 ) && ( sn = atoi( arg1 + 1 ) ) >= 0 ) || ( is_number( arg1 ) && ( sn = atoi( arg1 ) ) >= 0 ) ) )
	{
		struct skill_type *skill;
...
void do_feed( CHAR_DATA * ch, const char *argument )
{
	...
	if ( can_use_skill( ch, number_percent(  ), gsn_feed ) )
	{
		...
		if ( global_retcode == rNONE && !IS_NPC( ch ) && dam && ch->fighting && ch->pcdata->condition[COND_BLOODTHIRST] < ( 10 + ch->level ) )
		{
			gain_condition( ch, COND_BLOODTHIRST, UMIN( number_range( 1, ( ch->level + victim->level / 20 ) + 3 ), ( 10 + ch->level ) - ch->pcdata->condition[COND_BLOODTHIRST] ) );
	...
	else
	{
		...
		if ( global_retcode == rNONE && !IS_NPC( ch ) && ch->fighting && ch->pcdata->condition[COND_BLOODTHIRST] < ( 10 + ch->level ) )
		{
...
void do_broach( CHAR_DATA * ch, const char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg, TRUE ) ) != NULL )
	{
		...
		if ( !IS_SET( pexit->exit_info, EX_CLOSED )
			|| !IS_SET( pexit->exit_info, EX_LOCKED ) || IS_SET( pexit->exit_info, EX_PICKPROOF ) || can_use_skill( ch, number_percent(  ), gsn_broach ) )
		{
...
void do_scan( CHAR_DATA * ch, const char *argument )
{
	...
	if ( IS_AFFECTED( ch, AFF_BLIND ) && ( !IS_AFFECTED( ch, AFF_TRUESIGHT ) || ( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_HOLYLIGHT ) ) ) )
	{
...
ch_ret ranged_attack( CHAR_DATA * ch, const char *argument, OBJ_DATA * weapon, OBJ_DATA * projectile, short dt, short range )
{
	...
	while ( dist <= range )
	{
		...
		if ( IS_SET( pexit->exit_info, EX_CLOSED ) )
		{
			 ...
			 */
			if ( projectile )
				snprintf( buf, MAX_STRING_LENGTH, "You see your %s pierce a door in the distance to the %s.", myobj( projectile ), dir_name[dir] );
			else
			...
			if ( projectile )
			{
				snprintf( buf, MAX_STRING_LENGTH, "$p flies in from %s and implants itself solidly in the %sern door.", dtxt, dir_name[dir] );
				act( color, buf, ch, projectile, NULL, TO_ROOM );
			}
			else
			{
				snprintf( buf, MAX_STRING_LENGTH, "%s flies in from %s and implants itself solidly in the %sern door.", aoran( stxt ), dtxt, dir_name[dir] );
				buf[0] = UPPER( buf[0] );
