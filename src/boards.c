...
void write_board( BOARD_DATA * board )
{
	...
	else
	{
		for ( pnote = board->first_note; pnote; pnote = pnote->next )
		{
			fprintf( fp,
				"Sender  %s~\nDate    %s~\nTo      %s~\nSubject %s~\nVoting %d\nYesvotes %s~\nNovotes %s~\nAbstentions %s~\nText\n%s~\n\n",
				pnote->sender, pnote->date, pnote->to_list, pnote->subject, pnote->voting, pnote->yesvotes, pnote->novotes, pnote->abstentions, pnote->text );
		}
		fclose( fp );
	}
	return;
			...
			for ( pnote = board->first_note; pnote; pnote = pnote->next )
			{
				...
				if ( ( first_list && vnum >= first_list ) || !first_list )
					pager_printf( ch, "%2d%c %-12s%c %-24s : %-35s\r\n",
						vnum,
						is_note_to( ch, pnote ) ? ')' : '}', pnote->sender, ( pnote->voting != VOTE_NONE ) ? ( pnote->voting == VOTE_OPEN ? 'V' : 'C' ) : ':', pnote->date, pnote->subject );
			}
			if ( board->olistmessg )
			...
			for ( pnote = board->first_note; pnote; pnote = pnote->next )
			{
				...
				if ( ( first_list && vnum >= first_list ) || !first_list )
					pager_printf( ch, "%2d%c %-12s%c %-12.12s : %s\r\n",
						vnum,
						is_note_to( ch, pnote ) ? ')' : '}',
						pnote->sender, ( pnote->voting != VOTE_NONE ) ? ( pnote->voting == VOTE_OPEN ? 'V' : 'C' ) : ':', pnote->to_list, pnote->subject );
			}
			if ( board->olistmessg )
				act( AT_ACTION, board->olistmessg, ch, NULL, NULL, TO_CANSEE );
			else
	...
	if ( !str_cmp( arg, "read" ) )
	{
		...
		if ( !IS_MAIL )
		{
			...
			for ( pnote = board->first_note; pnote; pnote = pnote->next )
			{
				...
				if ( vnum == anum || fAll )
				{
					pager_printf( ch, "[%3d] %s: %s\r\n%s\r\nTo: %s\r\n%s", vnum, pnote->sender, pnote->subject, pnote->date, pnote->to_list, pnote->text );
...
void do_bstat( CHAR_DATA * ch, const char *argument )
{
	...
	ch_printf_color( ch,
		...
		board->note_file, board->board_obj, board->min_read_level, board->min_post_level, board->min_remove_level, board->max_posts, board->type, board->num_posts );
