...
void close_socket( DESCRIPTOR_DATA * dclose, bool force )
{
	...
	if ( !dclose->prev && dclose != first_descriptor )
	{
		...
		bug( "Close_socket: %s desc:%p != first_desc:%p and desc->prev = NULL!", ch ? ch->name : d->host, ( void * ) dclose, ( void * ) first_descriptor );
		dp = NULL;
		for ( d = first_descriptor; d; d = dn )
		{
			...
			if ( d == dclose )
			{
				bug( "Close_socket: %s desc:%p found, prev should be:%p, fixing.", ch ? ch->name : d->host, ( void * ) dclose, ( void * ) dp );
				dclose->prev = dp;
				break;
			}
			dp = d;
	...
	if ( !dclose->next && dclose != last_descriptor )
	{
		...
		bug( "Close_socket: %s desc:%p != last_desc:%p and desc->next = NULL!", ch ? ch->name : d->host, ( void * ) dclose, ( void * ) last_descriptor );
		dn = NULL;
		for ( d = last_descriptor; d; d = dp )
		{
			...
			if ( d == dclose )
			{
				bug( "%s: %s desc:%p found, next should be:%p, fixing.", __FUNCTION__, ch ? ch->name : d->host, ( void * ) dclose, ( void * ) dn );
				dclose->next = dn;
				break;
			}
			dn = d;
...
void read_from_buffer( DESCRIPTOR_DATA * d )
{
	...
	for ( i = 0, k = 0; d->inbuf[i] != '\n' && d->inbuf[i] != '\r'; i++ )
	{
		...
		if ( d->inbuf[i] == ( signed char ) IAC )
			iac = 1;
		else if ( iac == 1 && ( d->inbuf[i] == ( signed char ) DO || d->inbuf[i] == ( signed char ) DONT || d->inbuf[i] == ( signed char ) WILL ) )
			iac = 2;
		else if ( iac == 2 )
		{
			iac = 0;
...
void nanny_get_new_class( DESCRIPTOR_DATA * d, const char *argument )
{
	...
	if ( iClass == MAX_PC_CLASS || !class_table[iClass]->who_name || class_table[iClass]->who_name[0] == '\0' || !str_cmp( class_table[iClass]->who_name, "unused" ) )
	{
	...
	for ( iRace = 0; iRace < MAX_PC_RACE; iRace++ )
	{
		if ( iRace != RACE_VAMPIRE
			...
			&& !IS_SET( race_table[iRace]->class_restriction, 1 << ch->Class ) && str_cmp( race_table[iRace]->race_name, "unused" ) )
		{
			if ( iRace > 0 )
			{
...
void nanny_get_new_race( DESCRIPTOR_DATA * d, const char *argument )
{
	...
	if ( iRace == MAX_PC_RACE
		|| !race_table[iRace]->race_name || race_table[iRace]->race_name[0] == '\0'
		|| iRace == RACE_VAMPIRE || IS_SET( race_table[iRace]->class_restriction, 1 << ch->Class ) || !str_cmp( race_table[iRace]->race_name, "unused" ) )
	{
...
short check_playing( DESCRIPTOR_DATA * d, const char *name, bool kick )
{
	...
	for ( dold = first_descriptor; dold; dold = dold->next )
	{
		if ( dold != d && ( dold->character || dold->original ) && !str_cmp( name, dold->original ? dold->original->pcdata->filename : dold->character->pcdata->filename ) )
		{
			...
			log_printf_plus( LOG_COMM, UMAX( sysdata.log_level, ch->level ), "%s@%s reconnected, kicking off old link.", ch->pcdata->filename, d->host );
			d->connected = cstate;
			return TRUE;
		}
	}
